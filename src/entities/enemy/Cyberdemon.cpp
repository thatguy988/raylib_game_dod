...
namespace EnemySystem {
    ...
    Cyberdemon::Cyberdemon() : Enemy() {
        ...
        type = EnemyType::CYBER_DEMON; // Enemy type
    ...
    void Cyberdemon::CalculatePathToRandomTarget(Enemy& enemy, const std::vector<Vector3>& openPositions, int maze[MAX][MAX], int n, int m, float blockSize) {
        if (enemy.path.empty()) {
    ...
    void Cyberdemon::CalculatePathToPlayer(Enemy& enemy, const Vector3& playerPosition, int maze[MAX][MAX], int n, int m, float blockSize) {
    ...
    void Cyberdemon::MoveEnemyAlongPath(Enemy& enemy) {
        if (!enemy.path.empty()) {
    ...
    void Cyberdemon::Draw() const {
        DrawCube(position, 1.0f, 2.5f, 1.0f, GOLD);
    }
