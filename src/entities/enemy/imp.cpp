...
namespace EnemySystem {
    ...
    Imp::Imp() : Enemy() {
        ...
        type = EnemyType::IMP; // Enemy type
        enemyHealth = 100; // Health specific to Imp
    ...
    void Imp::CalculatePathToRandomTarget(Enemy& enemy, const std::vector<Vector3>& openPositions, int maze[MAX][MAX], int n, int m, float blockSize) {
        if (enemy.path.empty()) {
    ...
    void Imp::CalculatePathToPlayer(Enemy& enemy, const Vector3& playerPosition, int maze[MAX][MAX], int n, int m, float blockSize) {
    ...
    void Imp::MoveEnemyAlongPath(Enemy& enemy) {
        if (!enemy.path.empty()) {
    ...
    void Imp::HandleAttackState(Enemy& enemy, const Vector3& playerPosition, BulletSystem::BulletData& bulletManager, BulletSystem::SparseSet& set) {
    ...
    void Imp::UpdateEnemyState(Enemy& enemy, const Vector3& playerPosition, const std::vector<Vector3>& openPositions, int maze[MAX][MAX], int n, int m, float blockSize, bool rayHitsPlayer, BulletSystem::BulletData& bulletManager, BulletSystem::SparseSet& set) {
        switch (enemy.state) {
    ...
    void Imp::Draw() const {
        DrawCube(position, 1.0f, 2.5f, 1.0f, BLUE);
    }
