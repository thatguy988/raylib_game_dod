...
namespace EnemySystem {
    ...
    Demon::Demon() : Enemy() {
        ...
        movementSpeed = 0.15f; // Movement speed specific to Imp
        ...
        shootingInterval = 0.8f; // Shooting interval specific to Imp
        shootingHeightOffset = 1.0f; // Height offset for shooting
        type = EnemyType::DEMON; // Enemy type
        enemyHealth = 50; // Health specific to Imp
    ...
    void Demon::CalculatePathToRandomTarget(Enemy& enemy, const std::vector<Vector3>& openPositions, int maze[MAX][MAX], int n, int m, float blockSize) {
        if (enemy.path.empty()) {
    ...
    void Demon::CalculatePathToPlayer(Enemy& enemy, const Vector3& playerPosition, int maze[MAX][MAX], int n, int m, float blockSize) {
            enemy.lastKnownPlayerPos = playerPosition;
            enemy.path = Enemy::BFS(enemy.position, playerPosition, maze, n, m, blockSize);
            
    }
    
    void Demon::MoveEnemyAlongPath(Enemy& enemy) {
        if (!enemy.path.empty()) {
    ...
    void Demon::HandleAttackState(Enemy& enemy, bool playerEnemyCollision) {
        // Update the timer
        enemy.shootingTimer += GetFrameTime();

        // Check if there is a collision and if the timer has reached the attack interval
        if (playerEnemyCollision && enemy.shootingTimer >= enemy.shootingInterval) {
            // Apply damage to the player
            //ApplyDamageToPlayer(playerHealth, enemy.attackDamage);
            std::cout << "Damage the player" << std::endl;
    ...
    void Demon::UpdateEnemyState(Enemy& enemy, const Vector3& playerPosition, const std::vector<Vector3>& openPositions, int maze[MAX][MAX], int n, int m, float blockSize, bool rayHitsPlayer, bool playerEnemyCollision) {
        ...
        switch (enemy.state) {
            ...
            case EnemyState::ATTACKING:
                ...
                HandleAttackState(enemy, playerEnemyCollision);
    ...
    void Demon::Draw() const{
        DrawCube(position, 1.0f, 2.5f, 1.0f, YELLOW);
    }
